...
export function isNodeDone(node, progress) {
  ...
  if (node.type === 'dialog') return (progress.completedDialogs || []).includes(node.id)
  return false
}
export function pathNodeMeta(node) {
  ...
  if (node.type === 'dialog') { const d = DIALOGS.find(x => x.id === node.id); return { face: '話', label: d ? d.title : 'Gespräch', kind: 'Gespräch' } }
